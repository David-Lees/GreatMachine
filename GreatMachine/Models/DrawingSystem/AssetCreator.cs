...
namespace GreatMachine.Models.DrawingSystem
{
    ...
    public class AssetCreator
    {
        ...
        public Texture2D TextureFromShape(Shape shape, MaterialType type, Color color, float materialScale)
        {
            ...
            {
                ShapeType.Circle => CircleTexture(shape.Radius, type, color, materialScale, 24f),
                ShapeType.Polygon => TextureFromVertices(((PolygonShape)shape).Vertices, type, color, materialScale, 24f),
                _ => throw new NotSupportedException("The specified shape type is not supported."),
            };
        }
        ...
        public Texture2D EllipseTexture(float radiusX, float radiusY, MaterialType type, Color color, float materialScale, float pixelsPerMeter)
        {
            ...
            radiusX *= pixelsPerMeter;
            radiusY *= pixelsPerMeter;
            materialScale /= _materials[type].Width;
